...
<div class="space-y-8">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-gradient-card border border-gray-600 rounded-xl p-4 sm:p-6">
            <div class="space-y-4">
                <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-white">Criar Novo Módulo</h1>
                <p class="text-gray-300 text-sm sm:text-base">Crie um novo módulo para organizar seus vídeos educacionais.</p>
            </div>
        </div>
    </div>

    <div class="bg-gradient-card border border-gray-600 rounded-xl p-6">
        <form method="POST" action="{{ route('professor.videos.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="tema" name="tema" class="w-full px-4 py-3 rounded-lg bg-gray-800 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('tema') border-red-500 @enderror" required>
                        ...
                        @foreach($temas as $key => $label)
                            <option value="{{ $key }}" {{ old('tema') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('tema')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" id="aula_minima_acesso" name="aula_minima_acesso" value="{{ old('aula_minima_acesso', 1) }}" min="1" max="100"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descricao" name="descricao" rows="3"
                              ...
                              placeholder="Descreva o conteúdo deste módulo...">{{ old('descricao') }}</textarea>
                    @error('descricao')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-end">
                <a href="{{ route('professor.videos.index') }}" class="px-6 py-3 rounded-lg border border-gray-600 text-gray-300 hover:bg-gray-700 transition duration-150 text-center">Cancelar</a>
                <button type="submit" class="px-6 py-3 rounded-lg bg-gradient-blue text-white hover:opacity-95 transition duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500">Criar Módulo</button>
            </div>
        </form>
    </div>
</div>
