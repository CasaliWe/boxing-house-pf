...
@section('title', 'Adicionar Vídeo - ' . $modulo->titulo)
        ...
        <div class="bg-gradient-card border border-gray-600 rounded-xl p-4 sm:p-6">
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-white">Adicionar Vídeo</h1>
                </div>
                <p class="text-gray-300 text-sm sm:text-base">Adicione um novo vídeo ao módulo "{{ $modulo->titulo }}".</p>
            </div>
        </div>
    </div>
    
    <div class="bg-gradient-card border border-gray-600 rounded-xl p-6">
        <form method="POST" action="{{ route('professor.videos.store-video', $modulo) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label for="arquivo" class="block text-sm font-medium text-gray-300 mb-2">Arquivo de Vídeo</label>
                    <div class="relative">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descricao" name="descricao" rows="3"
                              ...
                              placeholder="Descreva o conteúdo deste vídeo...">{{ old('descricao') }}</textarea>
                    @error('descricao')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-end">
                ...
                <button type="submit" class="px-6 py-3 rounded-lg bg-gradient-blue text-white hover:opacity-95 transition duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500">Adicionar Vídeo</button>
            </div>
        </form>
    </div>

    <div class="bg-yellow-900/20 border border-yellow-600 rounded-xl p-4">
        <div class="flex items-start gap-3">
            <div class="text-yellow-400 text-xl">⚠️</div>
            <div class="text-yellow-200">
                <h3 class="font-semibold mb-1">Dicas para Upload de Vídeos:</h3>
                <ul class="text-sm space-y-1 text-yellow-100/90">
                    <li>• Use vídeos em formato vertical (9:16) para melhor experiência mobile</li>
                    <li>• Mantenha os vídeos curtos (até 5 minutos) para engajamento</li>
                    <li>• Comprima os vídeos antes do upload para reduzir o tamanho</li>
                    <li>• Use títulos claros e descritivos para os vídeos</li>
                </ul>
            </div>
        </div>
    </div>
